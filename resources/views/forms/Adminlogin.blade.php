	...
	<div class="main">
		...
		<div class="rsideAdmin">
			<div class="outterakbox">
				<form action="{{action('FormController@logindetails')}}" method="POST">
					{{csrf_field()}}
					<div class="head">
						<h2>LOG IN DETAILS ACCESS</h2>
					</div>
					@foreach($name as $key => $n)
					<div class="elementsAdminAbout">
						<p>User Name</p>
						<input type="text" name="UserName" value = "{{$n->UserName}}" id="FormInput" required>
					</div>
					<div class="elementsAdminAbout">
						<p>Password</p>
						<input type="password" name="Password" value="{{$n->Password}}" id="FormInput" required>
						@endforeach
					</div>	
